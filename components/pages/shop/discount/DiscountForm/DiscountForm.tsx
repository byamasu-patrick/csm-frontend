...
const DiscountForm: React.FC<ProductInfo> = ({productId, price, productName}) => {
    ...
    const DiscountElement = () => {
        return discountData.map((data, index) => {
            return (
                <option key={index} value={data} className='text-gray-900 py-4'>{data} %</option>
            )
        })
    }
    const addDiscount = async () => {
        ...
        if(description && title && discountPercentage){
            await dispatch(addDiscountToDB({
                description,
                headline: title,
                productId,
                productName,
            ...
            <div className="mt-4 w-full flex justify-between">
                <div className="space-y-1 w-full">
                    <label htmlFor="category" className="block text-sm font-medium text-gray-700">
                        Discount Percentage <small>(Please select the amount of percentage of discount from the actual price, this will generate a coupon to be applied when customers want to buy this product)</small>
                    </label>
                    <div className="mt-1">
                        <select
                            id="category"
