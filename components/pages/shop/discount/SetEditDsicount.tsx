...
const SetEditDiscount: React.FC<ProductProps> = (props) => {
    const [slide, setSlide] = useState<boolean>(true);
    const [addCart, setAddCart] = useState<number>(0);
    const router = useRouter()
    
    const dispatch = useAppDispatch();
    const { productsByCategory, isGetting, productsOwner } = useAppSelector(ProductSelector);
    ...
    return (
        <div>
            <div className="w-full py-6 flex sm:flex-col md:flex-col lg:flex-row xl:flex-row justify-center  bg-white ">                
                <div className="w-full lg:ml-6 mb-6">
                    <div className="relative">
                        <div className="slider">
                            <div className="slide-ana flex flex-shrink-0">
                                <div className={"flex flex-shrink-0 transform " + (slide ? "translate-x-0" : "-translate-x-full")}>
                                    <img className=" lg:block hidden w-full h-full object-center object-cover" src={props.data.imageFile} alt={props.data.name} />
                                    <img className=" hidden md:block lg:hidden w-full h-full object-center object-cover" src={props.data.imageFile} alt={props.data.name}/>
                                    <img className=" block md:hidden w-full h-full object-center object-cover" src={props.data.imageFile} alt={props.data.name} />
                                </div>
                        ...
                        <div className=" transition duration-150 absolute bottom-0 w-full h-full flex justify-between items-center px-4">
                            <button onClick={slideToggle} aria-label="previous" className="focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 rounded-full flex justify-center items-center">
                            ...
                            <button onClick={slideToggle} aria-label="Next" className=" focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 rounded-full flex justify-center items-center">
                                <svg className="" width="52" height="52" viewBox="0 0 52 52" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <circle cx="26" cy="26" r="26" fill="white" />
                ...
                <div className="lg:mt-0 flex justify-start items-start w-full flex-col mx-6 sm:px-3 mb-6">
                    <h2 className=" border-b pb-6 lg:text-2xl text-xl lg:leading-6 leading-5 text-gray-800 font-semibold">{props.data.name} | {props.data.category}</h2>
                    <p className="py-4">Provide all the necessary information about this discount, if it's promotion. Set up the deadline and percentage of discount you wish to have for this
                        particular product
                    </p>
                    <p className="py-4">
                        The discount will be made based on the percentage of the main price of the product
                    </p>
                    <div className=" flex justify-start items-center mt-4">
                        <p className="font-bold text-lg leading-6 text-gray-600 mr-4">MK {props.data.price}.00</p>
                    ...
                    <DiscountForm productId={props.data.id} productName={props.data.name} price={props.data.price}/>                    
                </div>                    
            </div>
            <style>
                {` 
...
export default SetEditDiscount;
