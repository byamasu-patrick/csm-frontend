...
const ReviewContent: React.FC<ProductReviewProps> = (props) => {
    const [menu, setMenu] = useState<boolean>(true);
    const dispatch = useAppDispatch(); 
    const [pageNumber, setPageNumber] = useState<number>(1);
    const { reviewsResponse } = useAppSelector(ProductReviewSelector);

    useEffect(() => {
        const getReviews = async () => {
            await dispatch(GetAllProductReviews({productId: props.productId, page: pageNumber}));
        };
    ...
    const getReviews = async () => {
        await dispatch(GetAllProductReviews({productId: props.productId, page: reviewsResponse.currentPage + 1}));
    };

    return (
        <div className="2xl:container 2xl:mx-auto flex justify-star items-start">
            <div className="flex flex-col justify-start items-start w-full space-y-8">
                <div className="flex justify-start items-start">
                    <p className="text-3xl lg:text-4xl font-semibold leading-7 text-gray-800">Reviews</p>
                </div>
                {
                    reviewsResponse.results.length > 0 ? (                        
                            ...
                            return (
                                <div className="w-full flex justify-start items-start flex-col">
                                    ...
                                    <div className={"md:block " + (menu ? "block" : "hidden")}>
                                        <p className="mt-3 text-base leading-normal text-gray-600 w-full md:w-9/12 xl:w-5/6">"{review.reviewText}"</p>
                                    
                                        <div className="mt-6 flex justify-start items-center flex-row space-x-2.5">                           
                                            <div className="flex flex-col justify-start items-start space-y-2">
                                                <p className="text-base font-medium leading-none text-gray-800 text-xl">{review.customerId}<span className="text-sm leading-none text-gray-600 ml-5">  {review.createdAt}</span></p>
                ...
                <button
                    onClick={() => getReviews().catch(error => console.log("An error has occured")) }
                    className="block w-full bg-gray-100 text-sm font-semibold hover:shadow-md hover:bg-orange-500 hover:text-white hover:shadow-xs p-3 my-4">Show
